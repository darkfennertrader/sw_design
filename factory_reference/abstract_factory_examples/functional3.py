...
def create_nl_tax_factory(
    tax_rate: float = 0.1, apply_floor: bool = True
) -> TaxFactory:
    return (
        partial(calculate_income_tax_nl, apply_floor=apply_floor),
        partial(calculate_tax_on_rate, tax_rate=tax_rate),
    )


def compute_tax(
    builder: Callable[[Optional[float], Optional[bool]], TaxFactory],
    income: int,
    capital: int,
    apply_floor: bool = True,
) -> int:
    """Computes tax for a given income and capital."""
    income_tax_calculator, capital_tax_calculator = builder(None, apply_floor)
